...
class Occasion(context: EventSourcedEntityContext) extends AbstractOccasion {
  ...
  private def onError(
      ...
      cmd: api.PublishOccasionCommand
  ): Option[String] =
    Option
      .when(currentState.id.length > 0)("Occasion already introduced")
      .orElse(
        Option.when(!isValidDateTimeString(cmd.startDatetimeIso8610Utc))(
  ...
  private def onError(
      currentState: OccasionState,
      cmd: api.VotePlaceForOccasionCommand
  ): Option[String] =
    Option
      .when(currentState.id.length == 0)("Occasion not existing")
      .orElse(
        Option.when(
          currentState.votes.find(_.voterUsername.equals(username)).nonEmpty
  ...
  override def publish(
      currentState: OccasionState,
      cmd: api.PublishOccasionCommand
  ): EventSourcedEntity.Effect[EntityId] =
  ...
  override def votePlace(
    ...
    onError(currentState, cmd) match {
      ...
      case None =>
        effects
          .emitEvent(
            VotePlacedForOccasion(
              occasionId = cmd.occasionId,
              placeId = cmd.placeId,
  ...
  override def occasionPublished(
      currentState: OccasionState,
  ...
  override def votePlacedForOccasion(
      currentState: OccasionState,
      event: VotePlacedForOccasion
  ): OccasionState =
    currentState.copy(votes =
      currentState.votes :+ Vote(
        placeId = event.placeId,
        voterUsername = event.voterUsername
      )
    )
