...
class Voter(context: EventSourcedEntityContext) extends AbstractVoter {
  ...
  private def onError(
      ...
      cmd: api.IntroduceVoterCommand
  ): Option[String] =
    Option
      .when(currentState.username.length > 0)("Voter already introduced")
      .orElse(
        Option.when(cmd.username.length < 0)(
          "Voter's username must be at least one character long"
        )
      )
      .orElse(
        Option.when(cmd.areaIds.size == 0)(
          "Voter needs to belong at least to one area"
        )
      )

  private def onError(
      currentState: VoterState,
      cmd: api.ArchiveVoterCommand
  ): Option[String] =
  ...
  override def voterIntroduced(
      currentState: VoterState,
      voterIntroduced: VoterIntroduced
  ): VoterState =
    currentState.copy(
      username = voterIntroduced.username,
      areaIds = voterIntroduced.areaIds,
      archived = false
    )

  override def voterArchived(
      currentState: VoterState,
      voterArchived: VoterArchived
  ): VoterState =
