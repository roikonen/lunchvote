...
import fi.simoroikonen.lunchvote.components.area.action.{ AreaExistsRequest, BoolResponse, PlaceExistsRequest }
...
class AreaActionsImpl(creationContext: ActionCreationContext) extends AbstractAreaActionsAction {
  ...
  override def areaExists(request: AreaExistsRequest): Action.Effect[BoolResponse] =
    effects.asyncReply(
  ...
  override def placeExists(request: PlaceExistsRequest): Action.Effect[BoolResponse] =
    effects.asyncReply(
      components.areasByIdView
        .getAreas(area.view.ById(request.organization, request.areaId))
        .execute()
        .map(_.areas.headOption
