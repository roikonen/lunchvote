...
class AdminControllerActionImpl(creationContext: ActionCreationContext)
  ...
  private def isAuthorized: Boolean = {
    ...
    if (!isAuthorized)
      log.warn(
        s"User '${usernameOption.getOrElse("UNDEFINED")}' is not authorized to act as an administrator"
      )
    isAuthorized
  }

  private def isValidUsername: Boolean =
    usernameOption.filter(_.equals("admin")).map(_ => true).getOrElse(false)
  ...
  private def onAuthError(): Future[Option[String]] =
    Future.successful(
      Option
        ...
        .orElse(
          Option.when(organizationOption.isEmpty)(
            "User's organization is not defined"
          )
        )
    )
  ...
  private def onError(cmd: IntroduceVoterCommand): Future[Option[String]] = {
    ...
    for {
      areasExists <- Future
        .sequence(
          cmd.areaIds.map(areaId =>
            organizationOption
              .map(areaExists(_, areaId))
  ...
  override def getAreaIds(empty: Empty): Action.Effect[GetAreaIdsResponse] =
    effects.asyncEffect(onAuthError().map {
      ...
      case None =>
        effects.forward(
          components.areaIdsView.getAreaIds(
            GetAreaIdsRequest(organization = organizationOption.get)
          )
        )
    })

  override def getAreasWithPlacesAndVoters(
      empty: Empty
  ): Action.Effect[AreasWithVotersResults] =
    effects.asyncEffect(onAuthError().map {
      case Some(error) => effects.error(error)
      case None =>
        effects.forward(
